...
class readData{
	...
	ros::Subsriber sub2; // subscribe to tracker_1: x,y,theta
...
readData::readData(){
 	sub2 = n.subscribe<geometry_msgs::Point>("/tracker_1", 1, &readData::callBack,this);
	sub = n.subscribe<geometry_msgs::Twist>("/keyboard",10, &readData::callBack,this);
	pub = n.advertise<geometry_msgs::Twist>("cmd_vel",1);
	pub2 = n.advertise<std_msgs::Float64MultiArray>("exp_data",1);
	std::stringstream ifile("/home/smanne1/catkin_ws/src/highBW/src/Cruise.csv"); 	
}
...
void readData::callBack2(const geometry_msgs::Twist::ConstPtr& msg){
	if (cruise == 1){
		if (initial == 0){
			x_i = msg->x; 
			y_i = msg->y;
	...
	x_f = (msg->x - x_i)*cos(theta_i) + (msg->y - y_i)*sin(theta_i) + 500;   // add the starting value of the robot instead of 500
	y_f = -(msg->x - x_i)*sin(theta_i) + (msg->y - y_i)*cos(theta_i) + 500;
